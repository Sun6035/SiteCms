...
function mark_learn(obj) {
    var learn = $(obj).attr('data-learn');
    if (learn == '1') {
        return false;
    }
    var id = $(obj).attr('data-id'), moduleid = $(obj).attr('data-moduleid');
    $.getJSON(APP + '?m=Ajax&a=learn_status&do=mark&id=' + id + '&moduleid=' + moduleid, function(data) {
        if(data.status=='1'){
            $(obj).text('已学习').attr('data-learn','1');
        }
    });
    return false;
}
...
$(function() {
    $('.J_learn').each(function() {
        var id = $(this).attr('data-id'), moduleid = $(this).attr('data-moduleid'), _this = $(this);
        $.getJSON(APP + '?m=Ajax&a=learn_status&id=' + id + '&moduleid=' + moduleid, function(data) {
            if (data.status == '1') {
                $(_this).text('已学习');
            }
