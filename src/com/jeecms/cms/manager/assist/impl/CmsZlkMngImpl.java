...
@Service
public class CmsZlkMngImpl implements CmsZlkMng {
	private static final Logger log = LoggerFactory.getLogger(CmsZlkMngImpl.class);
	...
	public void delTplAndRes(CmsSite site) throws IOException {
		...
		File res = new File(realPathResolver.get(site.getZlkPath()));
	...
	public List<FileEntry> export(CmsSite site, String solution) {
		...
		File res = new File(realPathResolver.get(site.getZlkPath()), solution);
		if (res.exists()) {
			for (File r : res.listFiles()) {
				fileEntrys.add(new FileEntry(FrontUtils.ZLK_EXP, r));
			}
		}
		return fileEntrys;
	}
	...
	@Override
	public List<FileWrap> listFile(String path, boolean dirAndEditable, String username) {
		File parent = new File(realPathResolver.get(path));
		if (parent.exists()) {
			File[] files;
			if (dirAndEditable) {
			...
			List<FileWrap> list = new ArrayList<FileWrap>(files.length);
			for (File f : files) {
				if(path.equals("/zlk/www")&&!username.equals("admin")){
					System.out.println(f.getName());
				if(	f.getName().equals(username)||f.getName().equals("pulic")){
				list.add(new FileWrap(f, realPathResolver.get("")));
				}
				}else{
