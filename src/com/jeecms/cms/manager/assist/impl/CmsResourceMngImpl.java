...
import static com.jeecms.core.web.util.FrontUtils.ZLK_EXP;
...
public class CmsResourceMngImpl implements CmsResourceMng {
	...
	public void imoport(File file, CmsSite site) throws IOException {
		String zlkRoot = site.getZlkPath();
		...
		while (en.hasMoreElements()) {
			...
			if (!entry.isDirectory()) {
				...
				// 模板还是资源
				if (name.startsWith(ZLK_EXP)) {
					filename = zlkRoot + "/" + solution
							+ name.substring(ZLK_EXP.length());
				} else {
